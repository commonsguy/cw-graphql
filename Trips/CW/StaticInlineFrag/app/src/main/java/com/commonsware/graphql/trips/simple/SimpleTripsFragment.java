...
import com.commonsware.graphql.trips.api.fragment.PlanFields;
...
public class SimpleTripsFragment extends RecyclerViewFragment
  ...
  private ApolloClient apolloClient=ApolloClient.builder()
    ...
    .build();
  private Observable<List<PlanFields>> observable;
  ...
  List<PlanFields> getAllTripsFields(Response<GetAllTrips.Data> response) {
    if (response.hasErrors()) {
      throw new RuntimeException(response.errors().get(0).message());
    }

    List<PlanFields> result=new ArrayList<>();

    for (GetAllTrips.AllTrip trip : response.data().allTrips()) {
      result.add(trip.fragments().planFields());
    }

    return(result);
  }

  List<PlanFields> getFindTripsFields(Response<FindTrips.Data> response) {
    if (response.hasErrors()) {
      throw new RuntimeException(response.errors().get(0).message());
    }
  ...
  private RecyclerView.Adapter buildAdapter(List<PlanFields> trips) {
  ...
  private static class TripsAdapter extends RecyclerView.Adapter<RowHolder> {
    private final List<PlanFields> trips;
    private final LayoutInflater inflater;
    private final DateFormat dateFormat;

    private TripsAdapter(List<PlanFields> trips,
